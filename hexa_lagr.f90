...
!----------------------------------------------------------------------
   integer,parameter ::     &
      ...
      nqd1 = 2                ! no. of gauss-quadrature points / dimension
...
!  1D Gaussian quadrature
   ...
   real(kind=rp),parameter,private :: 
      wg(maxint) = [                                & ! weights
         2._rp,                                     & ! 1 point
         1._rp, 1._rp,                              & ! 2 point
         5._rp/9._rp,8._rp/9._rp,5._rp/8._rp],      & ! 3 point
!         
...
!----------------------------------------------------------------------
   real(kind=rp),parameter,private,dimension(nqd1,nen1) ::                    &
      phi1    = product(xq - xa,3,mask)/product(xb - xa,3,mask),      &
      phi1_xi = sum(1./(xq - xa),3,mask)*N1             
...
!----------------------------------------------------------------------
   ...
   real(kind=rp),parameter,dimension(nsd,nen,nqd) ::                  &
      phi_xi = reshape([((((((phi_xi(i,p)*phi(j,q)*phi(k,r),          &
                              phi(i,p)*phi_xi(j,q)*phi(k,r),          &
                              phi(i,p)*phi(j,q)*phi_xi(k,r),          &
                        p = 1,nen1), q = 1,nen1), r = 1:nen1),        &
                        i = 1:nqd1), j = 1:nqd1), k = 1:nqd1)],       &
                        [nsd,nen,nqd])
